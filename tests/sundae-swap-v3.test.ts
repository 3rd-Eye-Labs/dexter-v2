import { SundaeSwapV3 } from '../src/dex/SundaeSwapV3.js';
...
test('fees returns protocolFee and deposit', () => {
    const dex = new SundaeSwapV3({} as any);
    const fees = dex.fees(baseParams as any);

    expect(fees.find(f => f.id === 'protocolFee')).toBeDefined();
