import { SundaeSwapV1 } from '../src/dex/SundaeSwapV1.js';
...
test('fees returns scooperFee and deposit', () => {
    const dex = new SundaeSwapV1({} as any);
    const fees = dex.fees(baseParams as any);

    expect(fees.find(f => f.id === 'scooperFee')).toBeDefined();
