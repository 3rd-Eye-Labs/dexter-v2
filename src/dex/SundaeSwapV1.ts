...
import { AddressDetails, Assets, Constr, Data, getAddressDetails, Script, UTxO } from '@lucid-evolution/lucid';
...
export class SundaeSwapV1 extends IDexterHelper implements SwapBuilder {
    ...
    buildSwapOrder(params: SwapBuilderParameters): Payment[] {
        const scooperFee: SwapFee | undefined = this.fees(params).find((fee: SwapFee) => fee.id === 'scooperFee');
        ...
        const assets: Assets = {
            lovelace: scooperFee.value + deposit.value + (swapToken === 'lovelace' ? swapAmount : 0n),
        };
        ...
        const datum = new Constr(0, [
            params.liquidityPool.identifier,
            new Constr(0, [
                new Constr(0, [
                    new Constr(0, [
                        new Constr(0, [
    ...
    estimatedGive(params: SwapBuilderParameters): bigint {
        ...
        const receive: bigint = (reserveIn * reserveOut) / (reserveOut - params.outAmount) - reserveIn;
        const swapFee: bigint = ((receive * BigInt(Math.floor(params.liquidityPool.state.sellFeePercent * 100))) + BigInt(10000) - 1n) / 10000n;
    ...
    estimatedReceive(params: SwapBuilderParameters): bigint {
        ...
        const swapFee: bigint = ((params.inAmount * BigInt(Math.floor(params.liquidityPool.state.buyFeePercent * 100))) + BigInt(10000) - 1n) / 10000n;

        return reserveOut - (reserveIn * reserveOut) / (reserveIn + params.inAmount - swapFee);
    }
